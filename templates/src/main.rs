...
use crate::{
    ...
    server::api::commands::{
        {{#each tables}}
        {{this.name}}_controller::{find_{{ this.name }}_list, delete_{{ this.name }}_ids, find_{{ this.name }}_by_id, update_{{ this.name }}, create_{{ this.name }}, create_{{ this.name }}_batch},
        {{/each}}
    },
    service::Service,
};
