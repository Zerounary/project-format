package com.cp.melon.adapter.utils;
 ...
 * 前后端查询参数工具类
 *
 * @author Aidy.Q@bitsun-inc.com
 * @since 2.0.0 2018-03-14 qp_fieldName_option_value
 */
public class QueryParamUtils {
        ...
        params.forEach((k, v) -> {
            if (k.startsWith(qpPrefix)) {
                ...
                if (v != null && StrUtil.isNotBlank(v.toString())) {
                    freshEw(ew, qpMap.get("option"), qpMap.get("fieldName"),
                            formatFieldVal(camelFieldName, v, clz, qpMap.get("option")));
                }
            }
        });
    ...
    public static QueryWrapper getMultiEntityWrapper(Map<String, Object> params, Class entity) {
        ...
        Map<String, Class> entities = MapUtil.newHashMap();
            ...
            if (k.startsWith(qpPrefix)) {
                ...
                if (v != null && StrUtil.isNotBlank(v.toString())) {
                    ...
                    if (otherTableFields != null && !otherTableFields.isEmpty() && otherTableFields
                        ...
                        freshEw(ew, qpMap.get("option"), key,
                                formatFieldVal(camelFieldName, v, clz, qpMap.get("option")));
                    } else {
                        Class clz = entities.get(dfPrex);
                        freshEw(ew, qpMap.get("option"), dfPrex + fieldName,
                                formatFieldVal(camelFieldName, v, clz, qpMap.get("option")));
                    }
                }
            }
    ...
    private static Map<String, String> getQueryParamMap(String paramStr) {
        Map retMap = MapUtil.newHashMap();
        paramStr = SQLFilter.sqlInject(paramStr);
        paramStr = StrUtil.removePrefix(paramStr, qpPrefix);
        String[] params = StrUtil.split(paramStr, splitStr);
        int len = 2;
    ...
    public static Object castType(String fieldNameType, Object val, String option) {
        ...
        String simpleTypeName = StrUtil.subAfter("." + fieldNameType, ".", true).toLowerCase();
        //判断是否是数组
        if (StrUtil.isNotBlank(option) && KEY_IN.contains(option) && !simpleTypeName.endsWith(BRACKETS)) {
...
//        Field[] baseFields = BasePo.class.getDeclaredFields();
        ...
        params.forEach((fieldName, val) -> {
            Optional<Field> field =
                    Arrays.stream(poFields).filter(f -> StrUtil.equals(f.getName(), fieldName)).findFirst();
//            if (!field.isPresent()) {
//                field = Arrays.stream(baseFields).filter(f -> StrUtil.equals(f.getName(), fieldName)).findFirst();
//            }
            if (!field.isPresent()) {
                throw new AppException("参数错误(不支持以下参数)：" + fieldName);
            }
    ...
    public static UpdateWrapper updateWrapper4Map(Supplier<?> po, Long id, Map<String, Object> params) {
        ...
        params.forEach((fieldName, val) -> {
            ...
            if (StrUtil.equals(fieldName, remarks)) {
